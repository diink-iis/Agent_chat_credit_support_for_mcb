...
`recall@5` = доля кейсов, где хотя бы один `referenced_document` попал в top-5
...
**Текущий результат (GigaChat, все 180 кейсов): recall@5 = 56.1% (101/180).**

| Категория | recall@5 | Комментарий |
|-----------|---------:|-------------|
| info | 82.2% | основной retrieval — высокий |
| transactional | 73.3% | основной retrieval — высокий |
| edge_conflict | 88.9% | коллизии: нужные пункты в выдаче |
| escalation_negative | 44.4% | поведенческая категория |
| edge_manipulation | 38.9% | поведенческая категория |
| edge_no_data | 22.2% | ответ = «нет данных», не зависит от документа |
| escalation_sales | 16.7% | gold-ссылка — политика маршрутизации, не контент |
| offtopic | 11.1% | агент должен отклонить, retrieval не нужен |

На категориях, где ответ реально лежит в документах (info, transactional,
edge_conflict), recall высокий. Низкие значения у поведенческих категорий
(offtopic, escalation_sales, manipulation) — это **ожидаемо**: там «правильная»
ссылка указывает на регламент поведения агента (эскалация, отказ), а не на смысловой
ответ, и решается логикой `classify`/`escalate` у Участников 2–3, а не RAG-поиском.
